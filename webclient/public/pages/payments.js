...
function renderDataList(rows, _view, empty, openMenuId2, options) {
  ...
  const allSelected = selectable && rows.every((row) => selectedIds2.has(row.id));
  ...
  const items = rows.map((row) => {
    const checked = Boolean(selectedIds2?.has(row.id));
...
function render() {
  ...
  setPageSubtitle(
    rows.length ? `${rows.length} ${activeView} ${noun}, ${compactCurrency(totalDue, user.preferredCurrency)} expected` : `No ${activeView} payments`
  );
  ...
  content.innerHTML = `${bulkBarHtml()}${renderDataList(
    pageRows.map((r) => {
      ...
      const paid = r.amount_paid != null ? `Paid ${money(Number(r.amount_paid), user.preferredCurrency)}` : "Unpaid";
      return {
        id,
        title: String(r.property_name || "Property"),
        subtitle: `${r.lender || "Lender"} \xB7 Due ${due}`,
        href: r.property_id ? `/property?id=${r.property_id}` : void 0,
