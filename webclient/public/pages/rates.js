...
function renderRates() {
  ...
  setPageSubtitle(
    rows.length ? `${rows.length} ${noun}, ${compactCurrency(outstanding, user.preferredCurrency)} outstanding` : "No active mortgages"
  );
  content.innerHTML = renderDataList(
    rows.map((m) => ({
      ...
      href: m.property_id ? `/property?id=${m.property_id}` : void 0,
      ...
      summarySub: m.fixed_rate_expiry ? `Fixed until ${formatDateDmY(m.fixed_rate_expiry)}${expiryHint(m.fixed_rate_expiry) ? ` \xB7 ${expiryHint(m.fixed_rate_expiry)}` : ""}` : money(Number(m.outstanding_balance), user.preferredCurrency) + " outstanding",
