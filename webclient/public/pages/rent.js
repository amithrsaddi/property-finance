  ...
  list.innerHTML = renderDataList(
    rows.map((row) => {
      ...
      return {
        ...
        subtitle: `${formatMonthYear(String(row.rental_period || ""))} \xB7 Due ${formatDateDmY(String(row.expected_payment_date || ""))}`,
        href: row.property_id ? `/property?id=${row.property_id}` : void 0,
...
async function loadRent() {
  ...
  setPageSubtitle(
    count ? `${count} ${noun}, ${compactCurrency(data.totals.expected, user.preferredCurrency)} expected \xB7 ${compactCurrency(data.totals.received, user.preferredCurrency)} received` : "No rent records for this period"
  );
  renderList();
}
